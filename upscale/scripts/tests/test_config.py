...
import pytest
from upscale_cli import agent_config
from upscale_cli.config import load_settings
from upscale_cli.errors import ConfigurationError
...
def test_load_settings_resolves_secret_env(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    config = tmp_path / "agent_config.toml"
    config.write_text(
        "[upscale]\nbase_url='http://example.test/'\napi_key_env='UPSCALE_TEST_KEY'\n",
        encoding="utf-8",
    )
    monkeypatch.setenv("UPSCALE_TEST_KEY", "secret")
