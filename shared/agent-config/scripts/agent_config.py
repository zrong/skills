...
Copy this module into a skill that needs TOML configuration. Keeping the
helper inside each skill lets that skill continue to work when installed by
itself, without depending on this repository's ``shared`` directory.
"""
...
class ConfigNotFoundError(FileNotFoundError):
    """Raised when no configuration file exists in the lookup chain."""
...
def find_git_root(start: str | Path) -> Path | None:
    """Return the nearest Git root at or above ``start``."""
...
def config_candidates(
    ...
    *,
    ...
    """Build the deduplicated lookup chain in priority order."""
...
def find_config(
    ...
    *,
    ...
    """Find a config file, honoring an explicit path when supplied."""
    working_dir = _absolute(cwd or Path.cwd(), base=Path.cwd())
    if path is not None:
...
def load_config(
    ...
    *,
    ...
    """Load the complete TOML document and return it with its source path.

    ``missing="empty"`` returns ``({}, None)`` when automatic discovery finds
    nothing. An explicit missing path always raises so configuration typos are
...
def load_section(
    ...
    **kwargs,
    ...
    """Load one top-level TOML table and return it with the source path."""
